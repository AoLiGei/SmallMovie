...
public class RenRenCallback extends ItemTouchHelper.SimpleCallback {
    ...
    public void onChildDraw(Canvas c, RecyclerView recyclerView, RecyclerView.ViewHolder viewHolder, float dX, float dY, int actionState, boolean isCurrentlyActive) {
        ...
        for (int i = 0; i < childCount; i++) {
            ...
            if (level > 0) {
                child.setScaleX(1 - OverLayCardLayoutManager.SCALE_GAP * level + fraction * OverLayCardLayoutManager.SCALE_GAP);

                if (level < OverLayCardLayoutManager.MAX_SHOW_COUNT - 1) {
                    child.setScaleY(1 - OverLayCardLayoutManager.SCALE_GAP * level + fraction * OverLayCardLayoutManager.SCALE_GAP);
                    child.setTranslationY(OverLayCardLayoutManager.TRANS_Y_GAP * level - fraction * OverLayCardLayoutManager.TRANS_Y_GAP);
                } else {
